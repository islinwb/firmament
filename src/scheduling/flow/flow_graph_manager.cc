  ...
  if (!res_node) {
    ...
    if (res_node->type_ == FlowNodeType::PU) {
      ...
      if (rd_ptr->num_slots_below() == 0) {
        LOG(INFO) << "=======rd_ptr->num_slots_below():" << rd_ptr->num_slots_below()
                  << "rtnd_ptr->resource_desc().max_pods(): " << rtnd_ptr->resource_desc().max_pods();
...
void FlowGraphManager::UpdateResToSinkArc(FlowGraphNode* res_node) {
  if (res_node->type_ == FlowNodeType::PU) {
    LOG(INFO) << "====UpdateResToSinkArc, res_node->resource_id_" << res_node->resource_id_;
