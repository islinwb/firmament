...
FlowGraphNode* CpuCostModel::GatherStats(FlowGraphNode* accumulator,
  ...
  if (accumulator->type_ == FlowNodeType::PU) {
    ...
    if (have_sample) {
      ...
      auto m_rtnd = latest_stats->mutable_topology_node();
      LOG(INFO) << "Running/idle task count+++++ m_rtnd->resource_desc().num_slots_below(): "<< m_rtnd->resource_desc().num_slots_below()
                << ", m_rtnd->resource_desc().num_running_tasks_below():" << m_rtnd->resource_desc().num_running_tasks_below() << ", machine_res_id:" << machine_res_id
                << "machine_res_id_pus_:" << m_rtnd->resource_desc().max_pods() << "uuid" << m_rtnd->resource_desc().uuid();
